...
class Ticket:
    ...
    def is_simple_lucky(self) -> bool:
        ...
        """
        ticket = [int(digit) for digit in self._number]
    ...
    def is_complex_lucky(self) -> bool:
        ...
        """
        odd_digits_sum = sum([int(digit) for digit in self._number if int(digit) % 2 != 0])
        even_digits_sum = sum([int(digit) for digit in self._number if int(digit) % 2 == 0])
    ...
    def is_complex_lucky_alternative(self) -> bool:
        ...
        otherwise False
        """
        odd_index_sum = sum([int(digit) for (index, digit) in enumerate(self._number) if index % 2 != 0])
        even_index_sum = sum([int(digit) for (index, digit) in enumerate(self._number) if index % 2 == 0])
